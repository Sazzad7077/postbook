const checkLoggedInUser = () => {
  ...
  const handleEditPost = async (postId) => {
    let postComments;
    try {
      let res = await fetch(`http://localhost:5000/getPost/${postId}`);
      post = await res.json();
    } catch (err) {
      console.log("error: ", err);
    } finally {
      document.getElementById('postId').value = post.postId;
      document.getElementById('new-post').value = post.postText;
      document.getElementById('new-post-img-url').value = post.postImageUrl;
      return post;
    }
  };
  ...
  const handleDeletePost = async (postId) => {
    try {
      let res = await fetch(`http://localhost:5000/delete-post/${postId}`, {
        method: 'DELETE'
      });
    } catch (err) {
